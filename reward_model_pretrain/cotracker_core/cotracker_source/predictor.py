...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        assert queries is not None or grid_size > 0, "queries or grid_size must be provided"
    ...
    def _compute_sparse_tracks(
        ...
        tracks, visibilities, confidence, track_former_input, _ = self.model.forward(
            video=video, queries=queries, iters=iters
        )
        # trim the support grid
        if add_support_grid:
            n_support = self.support_grid_size ** 2  # 36
            tracks = tracks[:, :, :-n_support] # (B, T, N, 2)
            visibilities = visibilities[:, :, :-n_support] # (B, T, N)
            confidence = confidence[:, :, :-n_support] # (B, T, N)
            track_former_input = track_former_input[:, :-n_support]  # (B, N, T, C)

        return tracks, visibilities, confidence, track_former_input
