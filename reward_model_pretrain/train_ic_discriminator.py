...
import transformers
import pdb
from raft_reward_utils import OpticalFlowDiscriminator, BrightnessConsistencyMapDiscriminator
...
import sys
import os
sys.path.append(os.path.dirname('./sea_raft_core/__init__.py'))
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if ic_loss_config is not None:
        if ic_loss_config.encoder_type == 'resnet':
            flow_discriminator = BrightnessConsistencyMapDiscriminator(encoder_type='resnet', **ic_loss_config.resnet_discriminator_config)
        elif ic_loss_config.encoder_type == 'vit':
            flow_discriminator = BrightnessConsistencyMapDiscriminator(encoder_type='vit', **ic_loss_config.vit_discriminator_config)
        else:
            raise ValueError(f"Unknown encoder_type={ic_loss_config.encoder_type}. Use 'resnet' or 'vit'.")
        if resume_ckpt is not None:
            print('resume from ckpt:', resume_ckpt)
            _ckpt = torch.load(resume_ckpt, map_location='cpu')
            _ckpt = _ckpt['discriminator_state_dict']
            msg = flow_discriminator.load_state_dict(_ckpt, strict=True)
            print('Loading pretrained checkpoint: ', msg)

        flow_discriminator.to(accelerator.device, dtype=weight_dtype)
        #print the number of parameters in Millions
        if accelerator.is_main_process:
            print('number of parameters: ', sum(p.numel() for p in flow_discriminator.parameters() if p.requires_grad)/1e6, 'M')


    # Add adapter and make sure the trainable params are in float32.
    if mixed_precision == "fp16":
        if ic_loss_config is not None:
            cast_training_params(flow_discriminator, dtype=torch.float32)
    
    if scale_lr:
        learning_rate = (learning_rate * gradient_accumulation_steps * train_batch_size * accelerator.num_processes)
    if ic_loss_config is not None:
        discriminator_optimizer = torch.optim.AdamW(
            flow_discriminator.parameters(),
            lr=learning_rate,
    ...
    train_dataset = DiscriminatorDataset(real_video_dir=ic_loss_config.real_video_dir, fake_video_dir=ic_loss_config.fake_video_dir)
    ...
    if ic_loss_config is not None:
        flow_discriminator, discriminator_optimizer = accelerator.prepare(flow_discriminator, discriminator_optimizer)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ### >>>> Training >>>> ###
            with accelerator.accumulate(flow_discriminator):
                real_videos = batch["real_video"].to(dtype=weight_dtype)
                generated_videos = batch["fake_video"].to(dtype=weight_dtype)

                # Fake pass (detach generator output is fine)
                fake_logits_disc = flow_discriminator(generated_videos.detach())
                ...
                # Real pass
                real_logits_disc = flow_discriminator(real_videos)
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(flow_discriminator.parameters(), max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                if ic_loss_config is not None:
                    log_info["ic_loss_d"] = d_loss.item()
                    log_info["ic_loss_d_real"] = loss_D_real.item()
                    log_info["ic_loss_d_fake"] = loss_D_fake.item()
                ...
                if accelerator.is_main_process and (global_step % checkpointing_steps == 0 or step == len(train_dataloader) - 1):
                    ...
                    if ic_loss_config is not None:
                        checkpoint_dict = {
                            ...
                            'discriminator_state_dict': unwrap_model(flow_discriminator).state_dict(),
                        ...
                        weight_name = f"ic_discriminator-{global_step}.ckpt"
                        save_directory = os.path.join(output_dir, 'ic_discriminator')
            ...
            if global_step > 0 and global_step % validation_steps == 0:
                ...
                with torch.no_grad():
                    for val_batch in val_dataloader:
                        ...
                        logits_disc = flow_discriminator(video)
...
if __name__ == "__main__":
    ...
    main(name=name, report_to=args.report_to, resume_ckpt=args.resume_ckpt, **config)