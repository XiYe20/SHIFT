...
def set_edm_ancestral(svd_pipeline):
    config = dict(svd_pipeline.scheduler.config)  # Convert FrozenDict to regular dict
    ...
    svd_pipeline.register_modules(scheduler = ancestral_scheduler)
...
class EDMAncestralScheduler(EulerDiscreteScheduler):
    """
    `EDMAncestralScheduler` is a modification of `EulerDiscreteScheduler`, implement the step function by ancestral sampling
    ...
    def step(
        ...
        """
        We still ignore the s_churn, i.e., set s_churn as 0
        ...
        # dt = self.sigmas[self.step_index + 1] - sigma_hat #dt is same as \dot{sigma(t)}
        # prev_sample = sample + derivative * dt
        # Cast sample back to model compatible dtype

        # -----------euler ancestral -----------------
        # copy from https://github.com/huggingface/diffusers/blob/main/src/diffusers/schedulers/scheduling_euler_ancestral_discrete.py
