...
class IndexOnlyDataset(Dataset):
    """Returns indices only; actual decoding is done during buffer recompute."""
...
class BufferVideoDataModule(pl.LightningDataModule):
    def __init__(
        ...
        train_video_path_json=None,
        val_video_path_json=None,
        target_vid_size=(49, 704, 1280),
        vid_data_type="mp4",
    ...
    def setup(self, stage=None):
        self.train_dataset = VideoDataset(
            videos_root_dir=self.videos_root_dir,
            video_prompt_json_file=self.train_video_path_json,
            target_vid_size=self.target_vid_size,
            vid_data_type=self.vid_data_type,
        )
        ...
        self.val_dataset = VideoDataset(
            videos_root_dir=self.videos_root_dir,
            video_prompt_json_file=self.val_video_path_json,
            target_vid_size=self.target_vid_size,
            vid_data_type=self.vid_data_type,
        )
    ...
    def train_dataloader(self):
        ...
        self.sampler = ChunkedSampler(
            ...
            shuffle=True,
            passes_per_epoch=self.passes_per_epoch,
        )
        return DataLoader(
            self.train_index_dataset,
            batch_size=self.batch_size,
            sampler=self.sampler,
            num_workers=0,   # indices only
            drop_last=True,
        )
...
class ListSampler(Sampler):
    """Lets recompute_buffer() point a DataLoader at a specific list of indices."""
...
# ==========================
# SHIFT: gradnorm + histogram logging helpers (copied from svd_shift)
# ==========================
def group_param_norms_by_transformer_block(param_norms, prefix_regex):
    """
    param_norms: dict[name -> norm] (norm can be float or tensor)
    prefix_regex: regex with ONE capture group for the "layer key"
    returns: dict[layer_key -> 1D CPU tensor of norms]
    """
    import re
    import torch
    from collections import defaultdict
...
def log_hist(tb_logger, tensors, stage="train", global_step=None):
    ...
    Works for TensorBoard SummaryWriter (tb_logger.add_histogram).
    """
...
def _downsample_btchw(x_btchw: torch.Tensor, r: int) -> torch.Tensor:
    # x: (B,T,C,H,W)
    if r <= 1:
        return x_btchw
    B, T, C, H, W = x_btchw.shape
    x = x_btchw.reshape(B * T, C, H, W)
    x = F.interpolate(x, size=(H // r, W // r), mode="bilinear", align_corners=False)
    return x.reshape(B, T, C, H // r, W // r)
...
if __name__ == '__main__':
    ...
    # import pdb; pdb.set_trace()
    directory = '/local_workspace/deformation_processed_480x832x49_fps24_25steps_5555/real_videos'
    create_video_json(directory, output_file="video_files.json", recursive=False, prompt_json_file='/path/to/datasets/MotionAlignmentDatasets/wisa80k_correct/wisa_80k_captions.json', num_examples=5555)
    
    train_output_json = 'wisa80k_deformation_480x832x49_fps24_25steps_train.json'
    val_output_json = 'wisa80k_deformation_480x832x49_fps24_25steps_val.json'
    split_train_val_json('./video_files.json', train_output_json, val_output_json, validation_ratio=0.0999, seed=42)
