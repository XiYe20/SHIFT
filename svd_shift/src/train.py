...
from utils import VideoDataModule, BufferVideoDataModule
from utils import load_config, ConfigViewerCallback, ManualLoRACallback, ManualModelCheckpoint
from shift_svd_trainer import ShiftTrainerSVD
...
def main():
    ...
    best_loss_callback = ManualModelCheckpoint(
        ...
        every_n_epochs=1,
        filename="best_{val_ave_rewards:.4f}_{epoch}",  # Include epoch and loss
        monitor="val_fake_ave_rewards",     # Monitor val_loss
    ...
    data_module = BufferVideoDataModule(config.batch_size,
    ...
    sft_svd_trainer = ShiftTrainerSVD(config)
    ...
    trainer = pl.Trainer(
        limit_train_batches=config.limit_train_batches if config.debug_mode else None, # for debug
        limit_val_batches=config.limit_val_batches if config.debug_mode else None, # for debug
        logger=logger,
